{include file="header3.tpl" title="Account Settings" meta_title="Account Settings - {$settings.site_name}"}
...
<p>Your account data has been updated successfully.</p>
{/if}
{if $errors}
...
{/if}
            ...
            <div class="settings_box">
                ...
                <div class="row">
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" class="form-control" name=fullname value="{$frm.fullname|default:$userinfo.name|escape:html}">
                    </div>
                    <div class="col-12 col-md-6">
                    ...
                    <div class="col-12 col-md-6">
                        <label>Email</label>
                        {if $settings.usercanchangeemail == 1}
                        <input type="text" class="form-control" name=email value="{$frm.email|default:$userinfo.email|escape:html}">
                        {else}
                        <input type="text" class="form-control" value="{$userinfo.email|escape:html}" readonly>
                        {/if}
                    </div>
                </div>
                <h3 class="mt-3">Password changen</h3>
                <div class="row">
                    <div class="col-12 col-md-6">
            ...
            <div class="settings_box">
                <h3>Payment infromation</h3>
                <div class="row">
                    {foreach item=ps from=$pay_accounts}
                    <div class="col-12 col-md-6">
                        <label class="d-flex"><img src="./img/pp/{$ps.id}.svg" class="img-fluid my-auto" alt=""><span class="my-auto">{$ps.name}</span></label>
                        {if $settings.usercanchangeegoldacc == 0 && $ps.account != ''}
                        <input type="password" class="form-control" value="{$ps.account|escape:html}" readonly>
                        {else}
                        <input type="text" class="form-control" name="pay_account[{$ps.id}]" value="{$frm.pay_account[$ps.id]|default:$ps.account|escape:html}" data-validate="{$ps.validate.func}" data-validate-{$ps.validate.func}="{$ps.validate[$ps.validate.func]}" data-validate-notice="{$ps.validate.notification|escape:html}">
                        {/if}
                    </div>
                    {/foreach}
                </div>
            ...
            <div class="text-center mt-4">
                <button class="custom_btn1 btn">Save</button>
            </div>
        </div>
    </div>
</form>
